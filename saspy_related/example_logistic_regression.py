...
# %%
sas.sasdata("df", "work").head()

# %%
sas.submitLST(
    f"""
PROC CONTENTS DATA=df OUT=_tmp;
run;
"""
)
...
create table _tmp as
    select
        x_bin,
        count(1) as n,
        mean(prob) as avg_prob
    from preds
    where x_bin not in (9,10)
    group by 1
    order by 1
    ;
quit;
...
df
# %%
sas.submitLST(f"proc print data = preds;run;")
