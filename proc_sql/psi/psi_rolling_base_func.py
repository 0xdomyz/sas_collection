...
proc sql;
    ...
    from _period_dist p
    full outer join _period_dist b
        on p.{psi_cat_var} = b.{psi_cat_var}
       and b.{row_col} = p.{row_col} - {time_interval}
       {join_cls_2}
    where p.{row_col} > {early_time};
...
if __name__ == "__main__":
    ...
    # %%
    # vanilla
    qry1 = make_psi_rolling_base_qry(
        tbl="work.heart2",
    ...
    # %%
    # by 1 factor
    qry2 = make_psi_rolling_base_qry(
        tbl="work.heart2",
        vari="weight_status",
        varis=["chol_status", "bp_status", "weight_status"],
        psi_cat_var="smoking_status",
    ...
    # %%
    # custom where
    qry3 = make_psi_rolling_base_qry(
        tbl="work.heart2",
        varis=["chol_status", "bp_status", "weight_status"],
        psi_cat_var="smoking_status",
        row_col="age_decile",
    ...
    # %%
    df22 = df2.pivot_table(index=["age_decile"], columns="Weight_Status", values="psi")
    df22.reset_index().plot(
        x="age_decile",
        kind="line",
    )
    df22
